...
public class MCLLinePoseProvider implements PoseProvider, MoveListener
{

	private static final int LINE_THRS = 30;
	 ...
	 */
	public MCLLinePoseProvider(MoveProvider mp, RangeScanner scanner, ColorSensor color,
			RangeMap map, int numParticles, int border, Pose initialPose)
	{
		
		this.numParticles = numParticles;
		this.border = border;
//		particles = new MCLLineParticleSet(map, numParticles, border);
		particles = new MCLLineParticleSet(map, numParticles, initialPose,
				10, 0);
	...
	public void setInitialPose(Pose aPose, float radiusNoise, float headingNoise)
	{
		...
		particles = new MCLLineParticleSet(map, numParticles, aPose, radiusNoise, headingNoise);
	}
	...
	public void setInitialPose(RangeReadings readings,float sigma)
	{
		...
		particles = new MCLLineParticleSet(map, numParticles,border,readings, 2*sigma*sigma,minWeight  );
		updated = true; 
	}
	 ...
	 */
	public MCLLineParticleSet getParticles()
	{
		return particles;
	}
  ...
  Required by MoveListener interface; does nothing
	...
	public boolean  update()
	{
		...
		// Sense Line
		boolean line = true;//color.getLightValue() <= LINE_THRS;
				
		return update(line);


	}
	private boolean update(boolean line) {
		if(debug)System.out.println("MCLPP Line update readings called ");
		updated = false;
		busy = true;
		
		boolean goodPose;

		goodPose = particles.calculateWeights(line, map);
	...
	public void addMoveProvider(MoveProvider mp) {
