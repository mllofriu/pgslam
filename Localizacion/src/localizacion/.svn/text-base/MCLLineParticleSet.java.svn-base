 ...
 *
 */
public class MCLLineParticleSet {
	// Constants
	...
	// Instance variables
	private float distanceNoiseFactor = 0.04f;
	private float angleNoiseFactor = 0.8f;
	private int numParticles;
	private MCLLineParticle[] particles;
	 ...
	 */
	public MCLLineParticleSet(RangeMap map, int numParticles, int border)
	{
		this.map = map;
		this.numParticles = numParticles;
		this.border = border;
		boundingRect = map.getBoundingRect();
		particles = new MCLLineParticle[numParticles];
	 ...
	 */
	public MCLLineParticleSet(RangeMap map, int numParticles, Pose initialPose,
...
//			float y = initialPose.getY() + rad;
			...
			particles[i] = new MCLLineParticle((new Pose(x,y,heading)));
			if(debug){
	 ...
	 * @return the particle
	 */
	private MCLLineParticle generateParticle() {
		float x, y, angle;
		...
		return new MCLLineParticle(new Pose(x, y, angle));
	}
	 ...
	 * @return the particle
	 */
	public MCLLineParticle getParticle(int i) {
		return particles[i];
	}
	...
	public boolean resample() {
		...
		MCLLineParticle[] oldParticles = particles;
		particles = new MCLLineParticle[numParticles];
		...
		while (count < numParticles) {
			...
			if (iterations >= maxIterations) {
				...
				if (count > 0) { // Duplicate the ones we have so far
					for (int i = count; i < numParticles; i++) {
						particles[i] = new MCLLineParticle(particles[i % count].getPose());
			...
			for (int i = 0; i < numParticles && count < numParticles; i++) {
				if (oldParticles[i].getWeight() >= rand) {
					...
					particles[count] = new MCLLineParticle(new Pose(x, y, angle));
	...
	private void normalizeWeights(MCLLineParticle[] particles) {
		// Encuentro el total
		float tWeight = 0;
		for(MCLLineParticle p : particles)
			tWeight += p.getWeight();
	...
	public boolean calculateWeights(boolean line, RangeMap map2) {
		int zeros= 0;
		maxWeight = 0f;
		for (int i = 0; i < numParticles; i++)
		{
			particles[i].calculateWeight(line, map, twoSigmaSquared);
			float weight = particles[i].getWeight();
			if (weight > maxWeight) maxWeight = weight;
			if(weight == 0 )zeros++;
		}
	...
	public void dumpParticles(DataOutputStream dos) throws IOException {
		...
		for (int i = 0; i < numParticles(); i++) {
			MCLLineParticle part = getParticle(i);
	...
	public void loadParticles(DataInputStream dis) throws IOException {
		...
		for (int i = 0; i < numParticles; i++) {
			...
			particles[i] = new MCLLineParticle(pose);
	...
	public void dumpClosest(RangeReadings rr, RangeMap map, DataOutputStream dos, float x, float y) throws IOException {
		...
		MCLLineParticle p = getParticle(closest);
	...
	public MCLLineParticle[] getParticles() {
		return particles;
	}
