...
public class MapaIEEE {
	...
	private final static float X1 = CULTIVO_ZONE_WIDTH+TAPE_SIDE*2;
	private final static float X2 = CULTIVO_ZONE_WIDTH+TAPE_SIDE*2 + ALBERCA_FREE_SPACE;
	private final static float X3 = CULTIVO_ZONE_WIDTH+TAPE_SIDE*2 + ALBERCA_FREE_SPACE +
		ALBERCA_ZONE_WIDTH + TAPE_SIDE * 2;
	private final static float X4 = ARENA_ZONE_WIDTH-BENEFICIADERO_ZONE_WIDTH-TAPE_SIDE*2;
	...
	public static LineMap getIEEEMap()
	{
		...
		lines.toArray(larray);
			//LineMap map = new LineMap(larray, new Rectangle(0.0f,0.0f,2000.0f,2400.0f));
		LineMap map = new LineMap(larray, new Rectangle(X0, Y0, X5, Y7));
		return map;
	}
